...
import { useDayRecords } from '../selectors';
...
export function InsightsScreen() {
  ...
  const completedThisWeek = useMemo(
    () => tasks.filter((t) => t.status === 'completed' && last7.includes(taskDate(t))),
    [tasks, last7]
  );
        ...
        {nextBest ? (
          <View style={[styles.nextBestCard, { backgroundColor: colors.primary }]}>
            ...
            <View style={styles.nextBestMeta}>
              <Text style={styles.nextBestMuted}>{taskCategory(nextBest)}</Text>
              <Text style={styles.nextBestMuted}>·</Text>
              <Text style={styles.nextBestMuted}>{nextBest.estimatedDurationMinutes}m</Text>
              <Text style={styles.nextBestMuted}>·</Text>
              <Text style={styles.nextBestMuted}>{nextBest.score} pts</Text>
            </View>
            <TouchableOpacity
              accessibilityRole="button"
              accessibilityLabel={`Start ${nextBest.title}`}
              style={styles.startBtn}
              onPress={() => {
                Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium);
                startSession(nextBest);
                router.push('/timer');
              }}
            >
