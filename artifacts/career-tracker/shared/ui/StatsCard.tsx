...
function StatsCardImpl({ label, value, sub, accent = false }: Props) {
  ...
  return (
    ...
    >
      ...
      <Text style={[styles.label, { color: accent ? 'rgba(255,255,255,0.8)' : colors.mutedForeground }]}>{label}</Text>
      {sub ? (
        <Text style={[styles.sub, { color: accent ? 'rgba(255,255,255,0.6)' : colors.mutedForeground }]}>{sub}</Text>
      ) : null}
    </View>
  );
}
